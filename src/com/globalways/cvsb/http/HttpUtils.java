...
public class HttpUtils implements HttpClientDao {
	...
	// 超时时间10秒
	private static AsyncHttpClient mHttpUtil = new AsyncHttpClient(TIMEOUT);
	/** 最大任务数 **/
	...
	public void sendGetRequest(final String url, final long flag, Map<String, Object> params,
		...
		HttpAuth auth = new HttpAuth("GET", url, params);
		Map<String, Object> signedParams = auth.getRequestParams();
...
//		}
		if (null != signedParams) {
			for (String key : signedParams.keySet()) {
				rp.put(key, signedParams.get(key));
			}
			MyLog.d(TAG, "get RequestParams = " + signedParams.toString());
		}
		MyLog.d(TAG, "get Request url = " + url);
		mHttpUtil.setTimeout(TIMEOUT);
	...
	public void sendPostRequest(final String url, final long flag, Map<String, Object> params,
			final HttpClientUtilCallBack<String> httpClientUtilCallBack) {
		HttpAuth auth = new HttpAuth("POST", url, params);
		Map<String, Object> signedParams = auth.getRequestParams();
...
//		}
		...
		MyLog.d(TAG, "post Request url = " + url);
		mHttpUtil.setTimeout(TIMEOUT);
		// 超时时间3秒
	...
	public void sendPutRequest(final String url, final long flag, Map<String, Object> params,
			final HttpClientUtilCallBack<String> httpClientUtilCallBack) {
		HttpAuth auth = new HttpAuth("PUT", url, params);
		Map<String, Object> signedParams = auth.getRequestParams();
		// 组合参数,默认utf-8
		RequestParams rp = new RequestParams();
//		if (null != params) {
...
//		}
		...
		mHttpUtil.put(url, rp, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
				...
				MyLog.d(TAG, "PutRequest onSuccess: " + response);
				if (null != httpClientUtilCallBack) {
					httpClientUtilCallBack.onSuccess(url, flag, response);
				}
			...
			@Override
			public void onFailure(int arg0, Header[] arg1, byte[] arg2, Throwable arg3) {
				MyLog.d(TAG, "PutRequest onFailure: " + arg1 + "|" + arg3.getMessage());
				if (null != httpClientUtilCallBack) {
					httpClientUtilCallBack.onFailure(url, flag, ErrorCode.EXCEPTION);
				}
	...
	public void sendDeleteRequest(final String url, final long flag, Map<String, Object> params,
			final HttpClientUtilCallBack<String> httpClientUtilCallBack) {
		HttpAuth auth = new HttpAuth("DELETE", url, params);
		Map<String, Object> signedParams = auth.getRequestParams();
		
		MyLog.d(TAG, "delete RequestURL = " + url);
		// 组合参数,默认utf-8
		RequestParams rp = new RequestParams();
//		if (null != params) {
//			for (String key : params.keySet()) {
//				rp.put(key, params.get(key));
//			}
//			MyLog.d(TAG, "delete RequestParams = " + params.toString());
//		}
		if (null != signedParams) {
			for (String key : signedParams.keySet()) {
				rp.put(key, signedParams.get(key));
		...
		// 超时时间3秒
		mHttpUtil.setTimeout(TIMEOUT);
		mHttpUtil.delete(null, url, null, rp, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
				...
				MyLog.d(TAG, "DeleteRequest onSuccess: " + response);
				if (null != httpClientUtilCallBack) {
					httpClientUtilCallBack.onSuccess(url, flag, response);
				}
			...
			@Override
			public void onFailure(int arg0, Header[] arg1, byte[] arg2, Throwable arg3) {
				MyLog.d(TAG, "DeleteRequest onFailure: " + arg1 + "|" + arg3.getMessage());
				if (null != httpClientUtilCallBack) {
					httpClientUtilCallBack.onFailure(url, flag, ErrorCode.EXCEPTION);
				}
	...
	public void uploadFile(String[] fileLocalPath, final String fileServerPath, final long flag,
		...
		if (Tool.isFileExist(fileLocalPath)) {
			...
			MyLog.d(TAG,
					"uploadFile fileLocalPath: " + fileLocalPath.toString() + " , fileServerPath: " + fileServerPath);
	...
	public void downloadFile(final String localFilePath, final String fileServerPath, final long flag,
			final HttpClientUtilCallBack<String> httpClientUtilCallBack) {
		if (null != localFilePath && !"".equals(localFilePath) && null != fileServerPath
				&& !"".equals(fileServerPath)) {
			// 确保文件保存的父目录已存在
