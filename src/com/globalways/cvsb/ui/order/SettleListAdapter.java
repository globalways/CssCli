...
public class SettleListAdapter extends BaseAdapter implements PagedList<SettleEntity>{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			mItemView.tvReturnAmount = (TextView) convertView.findViewById(R.id.tvReturnAmount);
...
//		mItemView.tvEndTime.setText(Tool.formatDate(list.get(position).getEnd_time()*1000));
		mItemView.tvSettleAmount.setText("￥"+ Tool.fenToYuan(list.get(position).getSerial_amount()));
		mItemView.tvSettleStatus.setText(SettleStatus.valueOf(list.get(position).getStatus()).getDesc());
		mItemView.tvReturnAmount.setText("￥"+ Tool.fenToYuan(list.get(position).getReturn_amount()));
		return convertView;
	}
	
	private class ItemView{
		private TextView tvSettleNo, tvEndTime, tvSettleAmount,tvReturnAmount, tvSettleStatus;
	}
